...
class DiaryCategoryList(DiaryMixin, generic.ArchiveIndexView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['heading'] = '「{}」 カテゴリのにっき'.format(self.category.name)
        return context
...
class DiaryYearList(DiaryMixin, generic.YearArchiveView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['heading'] = '{}年のにっき'.format(self.kwargs['year'])
        return context
...
class DiaryMonthList(DiaryMixin, generic.MonthArchiveView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['heading'] = '{}年{}月のにっき'.format(self.kwargs['year'], self.kwargs['month'])
        return context
